...
function makeArrow(from, to, color) {
  ...
  const arrowHelper = new THREE.ArrowHelper(dir, arrowPos, len*0.06, color, len*0.04, len*0.025);
  return arrowHelper;
}
...
export default function App() {
  ...
  const [sizeMult, setSizeMult] = useState(0.6);
    ...
    fetch("/lattice.json")
      ...
      .then(graph => {
        ...
        keys.forEach((vtc, idx) => {
          ...
          shardData[vtc] = { vtc, type: node.type, count: node.count, pairs: node.pairs?.length||0, step: idx };

          const geo = buildCrystalGeometry(vtc);
          const mat = new THREE.MeshPhongMaterial({
            color, emissive: new THREE.Color(color).multiplyScalar(0.2),
            shininess: 160, specular: 0xffffff,
            transparent: true, opacity: 0.88, side: THREE.DoubleSide,
          });
          const mesh = new THREE.Mesh(geo, mat);
          mesh.position.set(ox,oy,oz);
          const scale = 0.9 + node.count*0.12;
          mesh.scale.setScalar(scale * sizeMult);
                    mesh.userData = { vtc, step: idx, baseScale: scale };
          scene.add(mesh);
          meshes[vtc] = mesh;
          meshesRef.current = meshes;
        window.__setSizeMult = (v) => {
          setSizeMult(v);
          Object.values(meshes).forEach(m => {
            if (m.userData?.baseScale) {
              m.scale.setScalar(m.userData.baseScale * v);
            }
          });
        };
        ...
        keys.forEach((vtc, fromIdx) => {
          ...
          Object.entries(graph[vtc].next||{}).forEach(([next, weight]) => {
            ...
            const mat = new THREE.LineBasicMaterial({
              color: 0x004466, transparent: true,
        ...
        function animate() {
          ...
          Object.values(meshes).forEach(m => {
            ...
            if (m.userData?.baseScale) {
              m.scale.setScalar(m.userData.baseScale * sizeMult);
            }
          });
    ...
    <>
      ...
      {selected && (
        <div style={{
          ...
          background:"rgba(0,8,16,0.88)", border:"1px solid rgba(0,255,136,0.3)",
          borderRadius:8, padding:"12px 18px", color:"#00ff88",
          fontFamily:"monospace", fontSize:13, backdropFilter:"blur(8px)",
          maxWidth:360, boxShadow:"0 0 20px rgba(0,255,136,0.15)"
        }}>
      ...
      {hoveredEdge && (
        <div style={{
          ...
          background:"rgba(0,8,20,0.9)", border:"1px solid rgba(0,180,255,0.4)",
          borderRadius:6, padding:"8px 14px", color:"#00ccff",
          ...
          <div style={{fontSize:10,color:"#fff88"}}>{hoveredEdge.from?.slice(0,16)}... → {hoveredEdge.to?.slice(0,16)}...</div>
          <div style={{marginTop:4}}>weight: <span style={{color:"#fff"}}>{hoveredEdge.weight}</span></div>
        </div>
      )}

      <div style={{
        position:"fixed", bottom:24, left:"50%", transform:"translateX(-50%)",
        background:"rgba(0,8,16,0.85)", border:"1px solid rgba(255,255,255,0.1)",
        borderRadius:8, padding:"10px 20px", fontFamily:"monospace",
        fontSize:11, color:"rgba(255,255,255,0.5)", backdropFilter:"blur(8px)",
        display:"flex", alignItems:"center", gap:12
      }}>
        <span style={{color:"rgba(0,255,136,0.6)"}}>FORMATION</span>
        <input type="range" min={0} max={maxStep} value={playhead??maxStep}
          onChange={handleSlider}
          style={{width:200, accentColor:"#00ff88"}}
        />
        <span style={{color:"#fff"}}>{(playhead??maxStep)+1} / {maxStep+1}</span>
      </div>

      <div style={{
        position:"fixed", top:16, right:16, color:"rgba(0,255,136,0.4)",
        fontFamily:"monospace", fontSize:11, textAlign:"right", pointerEvents:"none"
      }}>
        GN SHARD SPACE<br/>
